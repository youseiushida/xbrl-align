...
| ガードレール | 6種 | **7種** |
| jppfs起点 | CK.yamlのqnameで固定 | LLMが選択（qnameなし） |
| 定義ファイル | `CK.yaml`（201キー） | `CK_llm.yaml`（注記のみ） |
...
## 標準キーの構成（CK.yaml: 201キー）

| 財務諸表 | キー数 | 主な科目 |
|---------|--------|---------|
| PL | 21 | 売上高、売上原価、売上総利益、研究開発費、営業利益、経常利益、当期純利益 等 |
| CI | 4 | 包括利益、その他の包括利益、親会社/非支配株主帰属 |
| BS | 89 | 流動資産（有価証券、売上債権、棚卸資産7区分）、固定資産（有形5区分、無形、投資）、繰延資産、流動負債、固定負債、純資産（株主資本内訳、包括利益累計額5区分） |
| CF | 37 | 営業CF（小計、利息受払含む）、投資CF、財務CF（リース債務返済含む）、換算差額 |
| SS | 50 | 純資産構成要素（Member）、変動事由（配当、新株発行、自己株式、振替等） |
...
with open("output/2025-11-01_alignment.json") as f:
...
| 指標 | 算出方法 |
|------|---------|
| ROE | ProfitLossAttributableToOwnersOfParent / ShareholdersEquity |
| ROA | ProfitLoss / Assets |
| EBITDA | OperatingIncome + DepreciationAndAmortizationOpeCF |
| 営業利益率 | OperatingIncome / NetSales |
| 自己資本比率 | ShareholdersEquity / Assets |
| D/Eレシオ | (ShortTermLoansPayable + LongTermLoansPayable + BondsPayable) / ShareholdersEquity |
| 流動比率 | CurrentAssets / CurrentLiabilities |
| FCF | NetCashProvidedByUsedInOperatingActivities - (PurchaseOfPropertyPlantAndEquipmentInvCF + PurchaseOfIntangibleAssetsInvCF) |
| YoY成長率 | 同キーの当期値 / 前期値（contextRefの期間で区別） |

## 検証

```bash
# IFRS同義語検証（Original）
uv run python scripts/validate_ifrs_synonyms.py output/2025-11-01_alignment.json

# LLM-first検証
uv run python scripts/validate_llm_alignment.py output_llm/2025-11-01_alignment.json
...
CK.yaml の qname を起点に:
  ...
  ├─ 全キー（確定済み含む）をLLMに送り追加同義語を収集
  ├─ ガードレール（6種）で検証
  ├─ GS foldでjppfs業種バリアントを展開（5段階フィルタ）
  └─ Post-pass: 他キーのrepresentativeに含まれるvariantsを除外
```
...
| 6 | 重複排他（同一jpigp概念がexactで複数キーに入らないか） |
| 7 | confidence閾値 (< 0.7 を棄却) |
...
```
CK.yaml                        # 標準キー定義（201キー、Originalパイプライン用）
CK_llm.yaml                    # 標準キー定義（LLM-first用、注記のみ・qnameなし）
main.py                        # Original CLIエントリポイント
main_llm.py                    # LLM-first CLIエントリポイント
output/                        # Original 出力
output_llm/                    # LLM-first 出力
docs/
  textblock_treasure_map.md    # TextBlock/非構造化データの抽出可能性マップ
scripts/
  validate_ifrs_synonyms.py    # IFRS同義語検証（Original用）
  validate_llm_alignment.py    # LLM-first検証
...
src/xbrl_align/                # Original パイプライン
  ...
  match/
    pipeline.py                # ラベルマッチ + LLM補完 + Post-pass
...
src/xbrl_align_llm/            # LLM-first パイプライン
  config.py
  ck_loader.py                 # 注記フィールド対応（qnameなし）
  llm_matcher.py               # LLM全件マッチング
  guardrails.py                # 7種のガードレール
